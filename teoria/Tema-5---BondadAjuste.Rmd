...
\right.
$$
O sea, sería la función de distribución que corresponderia a la variable aleatoria discreta $X_n$ con dominio $D_X=\{x_{(1)}< x_{(2)}<\cdots< x_{(n)}\}$ y función de probabilidad $P(X_n = x_{(i)})=\frac{1}{n}$, $i=1,\ldots,n$.
...
Si la hipótesis nula es cierta, la distribución de la variable aleatoria $D_n$ no depende de la variable $X$.

O dicho en otras palabras, la función de distribución de la variable aleatoria $D_n$ no depende de la función de distribución $F_X$.
