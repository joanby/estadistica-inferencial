...
$$
p_7  = P(X_0\geq 4.45)=P\left(Z\geq \frac{4.45-3.5}{0.7}\right)=P(Z\geq `r round((4.45-3.5)/0.7,3)`)=`r round(pnorm(4.45,3.5,0.7,lower.tail=FALSE),3)`F,
$$
donde $Z=N(0,1)$. Por tanto, $e_7 = n\cdot `r round(pnorm(4.45,3.5,0.7,lower.tail=FALSE),3)`=40\cdot `r round(pnorm(4.45,3.5,0.7,lower.tail=FALSE),3)`=`r round(40*pnorm(4.45,3.5,0.7,lower.tail=FALSE),2)`.$
...
\right.
$$
O sea, sería la función de distribución que correspondería a la variable aleatoria discreta $X_n$ con dominio $D_X=\{x_{(1)}< x_{(2)}<\cdots< x_{(n)}\}$ y función de distribución $F_{X_n}(x_{(i)})=P(X_n \leq x_{(i)})=\frac{i}{n}$, $i=1,\ldots,n$.
