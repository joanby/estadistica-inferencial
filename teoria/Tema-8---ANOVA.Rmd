...
plot(presión,tabla.datos.ANOVA$Incremento.celular,type="p",
     pch=18,cex=2,xlab="Nivel de CO2",ylab="% de massa celular")
```
...
plot(1:50,tabla.datos.ANOVA$Incremento.celular,type="p",pch=17,
     xlab="Número de microorganismo",ylab="% de massa celular",cex=0)
...
points(41:50,tabla.datos.ANOVA$Incremento.celular[seq(from=5,to=50,by=5)],pch=16,cex=1.5)
...
## El contraste ANOVA
Recordemos que supondremos que los datos vienen dados según la estructura siguiente:
...
* Para calcular la suma total de los datos del $i$-ésimo, usaremos la función `aggregate` de `R`:
```{r}
sumas.niveles = aggregate(tabla.datos.ANOVA$Incremento.celular ~ 
                            tabla.datos.ANOVA$nivel.Presión, FUN="sum")
sumas.niveles
```
</div>
...
## Ejemplo
<div class="example">
* Para calcular la media total de los datos del $i$-ésimo, podemos usar otra vez la función `aggregate` de `R`:
```{r}
medias.niveles = aggregate(tabla.datos.ANOVA$Incremento.celular ~ 
                            tabla.datos.ANOVA$nivel.Presión, FUN="mean")
medias.niveles
```
</div>
...
Nos separa la **variabilidad de los datos** respecto la **media global** denominada **Suma Total de Cuadrados** en dos variabilidades:

* La **variabilidad de los medias de cada grupo** respecto la **media global**. A dicha **variabilidad** la llamaremos **Suma de Cuadrados de los Tratamientos**. Interesa que dicha **variabilidad** sea pequeña para que la hipótesis nula de **igualdad de medias** sea cierta.
...
* $\displaystyle SS_{Total}=\sum_{i=1}^k\sum_{j=1}^{n_i} X_{ij}^2 - \frac{T_{\bullet\bullet}^2}{N}$.
...
```{r}
n1=length(tabla.datos.ANOVA$Incremento.celular[tabla.datos.ANOVA$nivel.Presión=="0.0"]);
n2=length(tabla.datos.ANOVA$Incremento.celular[tabla.datos.ANOVA$nivel.Presión=="0.083"]);
n3=length(tabla.datos.ANOVA$Incremento.celular[tabla.datos.ANOVA$nivel.Presión=="0.29"]);
n4=length(tabla.datos.ANOVA$Incremento.celular[tabla.datos.ANOVA$nivel.Presión=="0.50"]);
n5=length(tabla.datos.ANOVA$Incremento.celular[tabla.datos.ANOVA$nivel.Presión=="0.86"]);
N=n1+n2+n3+n4+n5;
...
```{r}
(SStr1 = SSTr1=sum(table(tabla.datos.ANOVA$nivel.Presión)*
                     (medias.niveles[,2]-media.muestral)^2))
(SStr=sum(sumas.niveles[,2]^2/table(tabla.datos.ANOVA$nivel.Presión))-(suma.total^2)/N)
```
...
En particular, se puede usar $MS_E$ para estimar la variancia común $\sigma^2$.
...
Sintetizemos cómo realizar el contraste ANOVA en 4 pasos:
...
* Tercer paso: calculamos el **estadísto de contraste** $F$: $F=\frac{MS_{Tr}}{MS_E}.$
...
## Ejemplo
<div class="example">
Lo primero que podríamos hacer con los datos del ejemplo que estamos trabajando es hacer un boxplot del incremento celular en función de la presión:

```{r,fig.align='center'}
boxplot(tabla.datos.ANOVA$Incremento.celular ~ tabla.datos.ANOVA$nivel.Presión,
...
Si hemos rechazado la hipótesis nula $H_0:\mu_1=\cdots =\mu_k$, el siguiento paso es averiguar 
cuáles son los niveles diferentes. 

O sea, hallar aquellas parejas $(\mu_i,\mu_j)$ para las que podamos decir que $\mu_i\neq \mu_j$.
...
## Test T de Bonferroni
El $p$-valor de cada contraste es $2P(t_{N-k}\geq |t_{i,j}|)$, donde $t_{i,j}$ es el valor que  toma la estadístico.

<l class="observ">Observación:</l> si se realizan $c$ contrastes a un nivel de significación $\alpha$, la probabilidad de Error de Tipo I a alguno de ellos es mayor que $\alpha$: de hecho, es $1-(1-\alpha)^c$.

Por ejemplo, en el ejemplo que hemos desarrollado del aumento de la masa corporal del microorganismo, si realizamos $c=\binom52=10$ contrastes con nivel de significación $\alpha =0.05$, ¡la probabilidad de Error de Tipo I a alguno de ellos es 
...
Por ejemplo, n el ejemplo que hemos desarrollado del aumento de la masa corporal del microorganismo, si realizamos los  $10$ contrastes, para obtener un nivel de significación global  $\alpha =0.05$, realizamos cada contraste con nivel de significación $\frac{0.05}{10}=0.005$.
...
```{r}
pares=rbind(c(1,2),c(1,3),c(1,4),c(1,5),
...
## Ejemplo
<div class="example">
A continuación nos preguntamos qué p-valores son menores que 0.05 para saber entre qué pares podemos aceptar que hay diferencias:
```{r}
pares[which(p.valores<0.005),c(1,2)]
```
...
## Ejemplo
<div class="example">
Vemos que en todos los pares se verifica que el p-valor es menor que 0.05.

Concluimos por tanto que tenemos indicios suficientes para rechazar que no hay diferencias entre dos medias de aumento de masa celular del microorganismo para dos niveles cualquiera de presión de $CO{}_2$.
</div>


## Ejemplo con `R`
<div class="example">
Para realizar la comparación de parejas con `R`, tenemos que usar la función `pairwise.t.test`:
```{r}
...
Observamos que todos los p-valores son menores que 0.05 llegando a la misma conclusión que antes.
...
## Ejemplo
<div class="example">
A continuación, averiguamos qué contrastes verifican $\widetilde{P}_{k}\leq 0.05$:

```{r}
pares.ord[which(pares.ord[,5]<=0.05),c(1,2)] 
```
...
## Ejemplo
<div class="example">
En este caso, también vemos que en todos los pares se verifica que el p-valor es menor que 0.05.

Concluimos lo mismo que en los contrastes realizados usando el método de Bonferroni: tenemos indicios suficientes para rechazar que no hay diferencias entre dos medias de aumento de masa celular del microorganismo para dos niveles cualquiera de presión de $CO{}_2$.
</div>


## Ejemplo con `R`

Para realizar la comparación de parejas con `R`, tenemos que usar la mima función `pairwise.t.test` que usábamos en los contrastes por parejas usando el método de Bonferroni pero cambiando el parámetro `p.adjust.method`:
```{r}
...
## Ejemplo con `R`
<div class="example">
`R` nos vuelve a mostrar una tabla donde las filas y las columnas son los niveles del factor y cuyos valores son los p-valores de los contrastes por parejas pero usando el método de Holm.
...
* Se considera cada par $Y$ de medias muestrales y se calcula el valor absoluto $D_Y$ de la  diferencia entre las dos medias y el número $p$ de medias que hay entre las dos (incluyéndo las dos medias que comparamos).
...
y con el nivel de significación $\alpha =0.05$: (usamos 40 como grados de libertad del error puesto que es el valor más
próximo a $N-k=45$)
...
## Ejemplo
<div class="example">
Vemos que según el test de Kolmogorov-Smirnov-Lilliefors, no podemos rechazar que las 5 subsmuestras correspondientes a cada nivel de presión sigan la distribución normal.

Dejamos como ejercicio al lector la comprobación de la normalidad usando los otros tests.
</div>
...
Estos bloques vienen a ser los aparejamientos de los datos en los **contrastes de medias dependientes**.

## Introducción
En un contraste de **bloques completos aleatorios**,

* Se han aparejado los individuos en **bloques.** (**bloques**)
...
Lo que hacemos es seleccionar algunos individuos (los **bloques**), pedir a cada uno
que corra, ande y recorra en bicicleta una distancia fijada, y determinar para cada individuo el número
...
Al aparejar cada individuo con él mismo, eliminamos el efecto de la variación individual.
...
\right\}
$$
donde $\mu_{i\bullet}$, $i=1,2,3$ representa el número media de Kcal.
...
## Modelo
La expresión matemática del modelo a estudiar consiste ahora en separar las diferencias de los datos respecto la **media global** en cuatro sumandos: la **media global**, las diferencias de las **medias de cada nivel** respecto de la **media global**, las diferencias de las **medias de cada bloque** respecto la **media global** y los errores residuales:
$$
...
* Las $k\cdot b$ observaciones constituyen muestras aleatorias independientes, cada una de medida $1$, de $k\cdot b$ poblaciones.
...
Las tablas siguientes no da las medias poblacionales de cada bloque/tratamiento dentro de cada grupo para los dos ejemplos.
...
```{r}
kilocal = c(1.4,1.1,0.7,1.5,1.2,0.8,
...
```{r}
(sumas.tratamientos = aggregate(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
                      tabla.datos.ANOVA.BLOQUES$tratamiento, FUN="sum"))
```
...
```{r}
(medias.tratamientos = aggregate(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
                      tabla.datos.ANOVA.BLOQUES$tratamiento, FUN="mean"))
```
...
```{r}
(sumas.bloques = aggregate(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
                      tabla.datos.ANOVA.BLOQUES$bloque, FUN="sum"))
```
...
```{r}
(medias.bloques = aggregate(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
                      tabla.datos.ANOVA.BLOQUES$bloque, FUN="mean"))
```
...
```{r}
(media.muestral = suma.total/dim(tabla.datos.ANOVA.BLOQUES)[1])
```
...
* La **variabiliad** debida a los **tratamientos**.
...
* Variabilidad total:
```{r}
(SST = suma.total.cuadrados-suma.total^2/dim(tabla.datos.ANOVA.BLOQUES)[1])
```
...
(SS.Tr = (1/num.bloques)*sum(sumas.tratamientos[,2]^2)-
    suma.total^2/dim(tabla.datos.ANOVA.BLOQUES)[1])
```
</div>
...
Como el p-valor es muy pequeño, concluimos que tenemos indicios suficientes para rechazar que las medias de los tratamientos sean iguales. O sea, la energía consumida en correr, pasear y montar en bicicleta no es la misma para los tres casos.
</div>
...
```{r}
summary(aov(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
              tabla.datos.ANOVA.BLOQUES$tratamiento + tabla.datos.ANOVA.BLOQUES$bloque))
```
</div>
...
```{r}
summary(aov(tabla.datos.ANOVA.BLOQUES$kilocal ~ 
              as.factor(tabla.datos.ANOVA.BLOQUES$tratamiento) 
            + as.factor(tabla.datos.ANOVA.BLOQUES$bloque)))
```
...
Expresado en términos de la variabilidad, la efectividad del diseño por bloques significa que la variabilidad debida a los bloques, $SS_{Bl}$, explica una parte importante de la varibilidad total, $SST$. 
...
Concretamente, para llevar a cabo el **ANOVA de dos vías**, consderaremos el caso más sencillo: **diseño completamente aleatorio** con **efectos fijos**:
...
La variable aleatoria $X_{ijk}$, $i=1,\ldots,a$, $j=1,\ldots,b$, $k=1,\ldots,n$, nos da la respuesta de la $k$-ésimoa unidad
...
fotoperiodos = factor(rep(c(9,14),10))
...
* Suma y media de los datos para la combinación de niveles $i$ y $j$: $T_{ij\bullet}=\sum\limits_{k=1}^n X_{ijk}$, $\overline{X}_{ij\bullet}=\dfrac{T_{ij\bullet}}{n}$.
 
* Suma y media de los datos para el nivel $i$-ésimo: $T_{i\bullet\bullet}=\sum\limits_{j=1}^b\sum\limits_{k=1}^n X_{ijk}=\sum\limits_{j=1}^b T_{ij\bullet}$,   $\overline{X}_{i\bullet\bullet}=\dfrac{T_{i\bullet\bullet}}{bn}$.
...
* Suma y media de los datos para el nivel $j$-ésimo: $T_{\bullet j\bullet}=\sum\limits_{i=1}^a\sum\limits_{k=1}^n X_{ijk}=\sum\limits_{i=1}^a T_{ij\bullet}$, 
$\overline{X}_{\bullet j\bullet}=\dfrac{T_{\bullet j\bullet}}{an}$.
...
```{r}
(suma.combinación.niveles = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$temperatura+tabla.datos.GSI$fotoperiodos, 
                            FUN="sum"))
```
...
```{r}
(media.combinación.niveles = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$temperatura+tabla.datos.GSI$fotoperiodos, 
                            FUN="mean"))
```
...
```{r}
(suma.fotoperiodos = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$fotoperiodos, FUN="sum"))
(media.fotoperiodos = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$fotoperiodos, FUN="mean"))
```
...
```{r}
(suma.temperatura = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$temperatura, FUN="sum"))
(media.temperatura = aggregate(tabla.datos.GSI$GSI ~ 
                            tabla.datos.GSI$temperatura, FUN="mean"))
```
...
* Media muestras de todos los datos:
```{r}
...
```{r,fig.align='center'}
boxplot(tabla.datos.GSI$GSI ~ tabla.datos.GSI$fotoperiodos,xlab="Fotoperíodo",ylab="GSI")
```
...
```{r,fig.align='center'}
boxplot(tabla.datos.GSI$GSI ~ tabla.datos.GSI$temperatura,xlab="Temperatura",ylab="GSI")
```
...
```{r,fig.align='center'}
boxplot(tabla.datos.GSI$GSI ~ tabla.datos.GSI$fotoperiodos+tabla.datos.GSI$temperatura,
...
fotoperiodo=tabla.datos.GSI$fotoperiodos
temperatura=tabla.datos.GSI$temperatura
summary(aov(GSI ~ fotoperiodo*temperatura))
```
...
## Ejemplo
<div class="example">
Para dibujar el gráfico de interacción de la temperatura según el fotoperiodo, hacemos lo siguiente:
```{r,fig.align='center'}
interaction.plot(temperatura,fotoperiodo,GSI,xlab="fotoperiodo",ylab="GSI")
```
</div>
