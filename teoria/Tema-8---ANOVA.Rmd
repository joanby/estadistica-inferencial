...
Es un test similar al **t-test** pero el **estadístico de contraste** tiene una distribución diferente.

Par aplicar el **contraste de Tukey** en `R` hay que usar la función `TukeyHSD` y aplicarla al resultado de haber aplicado la función `aov`:
```{r,eval=FALSE}
TukeyHSD(aov(X~F))
```
...
```{r,fig.align='center'}
interaction.plot(fotoperiodo,temperatura,GSI,xlab="fotoperíodo",ylab="GSI")
```
</div>
...
```{r,fig.align='center'}
interaction.plot(temperatura,fotoperiodo,GSI,xlab="fotoperíodo",ylab="GSI")
```
</div>
