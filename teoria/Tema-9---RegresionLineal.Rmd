...
bptest(lm1, ~ X + I(X^2))
```
donde `lm1` es el objeto de `R` donde hemos guardado la información de la regresión original y `X` es la matrix que contiene los valores de la muestra de variables independientes.
...
Cuando nos encontramos en una situación como la del ejemplo de los bebés, en lugar de aplicar el **Test de White** para contrastar la **homocedasticidad** de los residuos, podemos aplicat el **Test de Breuch-Pagan**.
...
```{r}
y=c(57.5,52.8,61.3,67,53.5,62.7,56.2,68.5,69.2)
reg.mul.original = lm(y~X[,2]+X[,3]+X[,4]+X[,5])
...
Decidimos si hay autocorrelació positiva:
...
Decidimos si hay autocorrelació negativa:
...
La idea principal es verificar que no haya **interacción** entre las variables independientes y así, cada una tendrà un efecto aditivo en el modelo. 
...
summary(lm(y~X[,2]+X[,3]+X[,4]+X[,5]+valores.ajustados2))[[4]]
```
Vemos que la variable $\hat{y}_i^2$ no es significativa del modelo con un p-valor de `r summary(lm(y~X[,2]+X[,3]+X[,4]+X[,5]+valores.ajustados2))[[4]][6,4]`. Concluimos por tanto que no tenemos evidencias suficientes para rechazar la aditividad del modelo.
</div>
...
$$e_{ij}=e_i+b_jx_{ij},$$
donde $e_i$  es el residuo $i$-ésimo de la regresión lineal, $b_j$ és el coeficiente de $X_j$ en la regresión original y $x_{ij}$ es la observación $j$-ésima del individuo $i$-ésimo. 

## Linealidad: gráficos de residuos parciales

Los residuos parciales se dibujan contra els valors de $x_j$ y se hace su recta de regresión. 
...
Si la componente $(i,j)$ de la **matriz Hat**, $h_{ij}$ es grande, la observación $i$-èsima tiene un impacto sustancial en el valor predicho $j$-ésimo.
...
* En la regressió múltiple, $h_i$ mide la distancia de una observación al vector medio de $X$ de una forma parecida a la anterior.
...
## Observaciones influentes: Distancia de Cook
