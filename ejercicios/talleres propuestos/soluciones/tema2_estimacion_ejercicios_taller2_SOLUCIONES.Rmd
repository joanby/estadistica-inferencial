...
Nos dicen  que los datos de la muestra provienen de una población modelada por una  v.a. $X$ con distribución $U(0,b)$. Entonces $E(X)=\frac{b-0}{2}=\frac{b}{2}.$  Utilizaremos el el  método de los momentos  estimamos $E(X)$ por $\overline{X}$ luego $\frac{b}{2}=E(X)$ de donde $b=E(X)\cdot 2$ y por lo tanto  un estimador del parámetro  $b$ es $\hat{b}=2*\overline{X}.$ En nuestro caso y con R
